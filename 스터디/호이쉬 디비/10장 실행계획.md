...
```
-- 더미 데이터 삽입을 위해 FOREIGN_KEY_CHECKS를 일시적으로 비활성화
SET FOREIGN_KEY_CHECKS = 0;

-- member 테이블에 더미 데이터 삽입
INSERT INTO member (email, name, password, role) VALUES
('admin1@example.com', 'Admin One', 'password1', 'ADMIN'),
('user1@example.com', 'User One', 'password1', 'USER'),
('user2@example.com', 'User Two', 'password2', 'USER'),
('user3@example.com', 'User Three', 'password3', 'USER');

-- theme 테이블에 더미 데이터 삽입
INSERT INTO theme (description, name, thumbnail) VALUES
('Theme One Description', 'Theme One', 'thumbnail1.jpg'),
('Theme Two Description', 'Theme Two', 'thumbnail2.jpg');

-- reservation_time 테이블에 더미 데이터 삽입
...
INSERT INTO reservation (date, is_deleted, member_id, theme_id, time_id) VALUES
('2024-08-01', 0, 2, 1, 1),
('2024-08-01', 0, 3, 2, 2),
('2024-08-02', 0, 2, 1, 3),
('2024-08-02', 0, 4, 2, 4);

-- waiting 테이블에 더미 데이터 삽입 (10,000개 행)
DELIMITER $$
CREATE PROCEDURE insert_waiting_data()
BEGIN
    DECLARE i INT DEFAULT 1;
    WHILE i <= 10000 DO
        INSERT INTO waiting (is_deleted, member_id, reservation_id) VALUES
        (0, FLOOR(2 + RAND() * 3), FLOOR(1 + RAND() * 4));
        SET i = i + 1;
    END WHILE;
END $$
DELIMITER ;

CALL insert_waiting_data();

-- 절차 삭제 (선택 사항)
DROP PROCEDURE IF EXISTS insert_waiting_data;
...
```
SET @member_id = 3; -- 실제 member_id 값으로 변경
