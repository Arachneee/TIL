...
docker run --name mysql-container -v waitingVolume:/var/lib/mysql/  -e MYSQL_ROOT_PASSWORD=1234 -d -p 3306:3306 mysql:latest
...
BEGIN
    ...
    DECLARE max_members INT DEFAULT 99999;
    DECLARE max_themes INT DEFAULT 10;
    DECLARE max_times INT DEFAULT 10;
    DECLARE max_reservations INT DEFAULT 10;
    DECLARE reservation_id INT;

    -- 1. member 테이블에 10,000개 데이터 삽입
    WHILE i <= max_members DO
        INSERT INTO member (email, name, password, role)
        VALUES (
            CONCAT('user', LPAD(i, 5, '0'), '@example.com'),
            CONCAT('Name', LPAD(i, 5, '0')),
            'password',
            'USER'
        );
        SET i = i + 1;
    ...
    -- 2. theme 테이블에 10개 데이터 삽입
    SET i = 1;
    WHILE i <= max_themes DO
        INSERT INTO theme (description, name, thumbnail)
        VALUES (
            CONCAT('Description for theme ', i),
            CONCAT('Theme ', i),
            CONCAT('thumbnail_', i, '.jpg')
        );
        SET i = i + 1;
    END WHILE;

    -- 3. reservation_time 테이블에 10개 데이터 삽입
    SET i = 1;
    WHILE i <= max_times DO
        INSERT INTO reservation_time (start_at)
        VALUES (
            ADDTIME('00:00:00', CONCAT(i, ':00:00'))
        );
        SET i = i + 1;
    END WHILE;
    
    -- 4. reservation 테이블에 10개 데이터 삽입
    SET i = 1;
    WHILE i <= max_reservations DO
        INSERT INTO reservation (date, is_deleted, member_id, theme_id, time_id)
        VALUES (
            CURDATE() + INTERVAL (i - 1) DAY,
            FALSE,
            i,
            i,
            i
    ...
    -- 5. waiting 테이블에 각 reservation_id에 대해 10,000개 데이터 삽입
    SET i = 1;
    WHILE i <= max_reservations DO
        -- Get the current reservation_id
        SET reservation_id = i;
        
        SET j = 1;
        WHILE j <= 99999 DO
            INSERT IGNORE INTO waiting (is_deleted, member_id, reservation_id)
            VALUES (
                FALSE,
                j,
                reservation_id
...
```
SET @member_id = 5000; -- 실제 member_id 값으로 변경
SET @date = '2024-08-01'; -- 실제 날짜 값으로 변경

EXPLAIN ANALYZE SELECT 
    w.reservation_id,
    COUNT(w2.id) AS waiting_count,
...
FROM 
    waiting w
INNER JOIN 
    waiting w2 ON w.reservation_id = w2.reservation_id
    AND w2.id <= w.id
INNER JOIN 
    reservation r ON w.reservation_id = r.id
INNER JOIN 
    reservation_time rt ON r.time_id = rt.id
INNER JOIN 
...
WHERE 
    ...
    AND r.date >= @date
GROUP BY 
    w.reservation_id, r.date, rt.start_at, t.name;
```


0.14
```
SET @member_id = 88888; -- 실제 member_id 값으로 변경
SET @date = '2024-08-01'; -- 실제 날짜 값으로 변경
...
EXPLAIN ANALYZE SELECT
    ...
    (SELECT count(w2.id)
    FROM waiting w2
    WHERE w2.reservation_id = w.reservation_id AND w2.id <= w.id) AS waiting_count
FROM waiting w
INNER JOIN
    reservation r ON w.reservation_id = r.id	
WHERE 
...
GROUP BY
    w.reservation_id;
```
![[Pasted image 20240804204001.png]]
![[Pasted image 20240804205123.png]]
...
GROUP BY 
    w.reservation_id;
```
![[Pasted image 20240804205105.png]]
![[Pasted image 20240804205050.png]]

결론
- inner join 을 사용하게 되면 group by 전에 w2.id <= w.id 조건으로 중복이 제거되지 않은 임시테이블을 생성하고 이후 group by하면서 임시테이블을 카운팅하게 된다.
- 반면에 서브쿼리를 사용하면 group by로 중복이 제거된 상태의 임시테이블이 만들어지고 이 임시테이블을 기준으로 서브쿼리가 돌아가는데 해당 서브쿼리는 count()쿼리와 primary 키 비교로 디스크 I/O 없이 매우 빠르게 count 할 수 있다.